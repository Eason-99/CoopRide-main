...
def running_example(args, training_round=1400, fleet_help=False):
    ...
    for iteration in range(training_round):
        ...
        while T < max_iter:
            ...
            # 执行一步环境交互
            # 参数: orders-选中的订单, generate_order=1-生成新订单
            # serve_drivers_ids, states_node, states, order_list, order_idx, order_feature, global_order_states = env.step(orders, generate_order=1, mode='')
            tmp = env.step(orders, generate_order=1, mode='')
